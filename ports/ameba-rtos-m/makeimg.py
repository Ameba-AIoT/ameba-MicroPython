...
def main():
    if len(sys.argv) != 5:
        print(f"usage: {os.path.basename(sys.argv[0])} <.config> <boot.bin> <app.bin> <firmware.bin>")
        sys.exit(1)
    ...
    if app_phys < boot_size:
        print(
            f"error: app region (0x{app_phys:06x}) overlaps boot region "
            f"(size 0x{boot_size:06x})"
        )
        sys.exit(1)
    ...
    print(f"           boot @ 0x{boot_phys:06x}  {len(boot_data):>7} B  "
